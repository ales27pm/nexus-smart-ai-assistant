...
function resolveSafeTimeZone(): string {
  if (
  ...
  try {
    ...
    console.warn("Failed to resolve timezone from Intl API", error);
  }

  return "UTC";
}

function isValidIosUdid(udid: string): boolean {
  if (!/^[a-fA-F0-9-]+$/.test(udid)) {
    return false;
  }

  // Common iOS UDID patterns from Xcode/devicectl output:
  // - 40 hex chars (legacy)
  // - 8-16 style dashed segments (e.g. 00008110-001C195A0E91001E)
  if (/^[a-fA-F0-9]{40}$/.test(udid)) {
    return true;
  }

  if (/^[a-fA-F0-9]{8}-[a-fA-F0-9]{16}$/.test(udid)) {
    return true;
  }

  if (/^[a-fA-F0-9]{25}$/.test(udid)) {
    return true;
  }
...
export function buildRviCaptureCommands(udid: string): string[] {
  ...
  if (!isValidIosUdid(normalizedUdid)) {
    throw new Error(
      "UDID must be a valid iOS identifier using hexadecimal characters and dashes",
    );
  }
