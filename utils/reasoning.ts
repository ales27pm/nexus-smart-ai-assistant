...
const BIAS_PATTERNS: Array<{
  ...
  {
    pattern: /\b(everyone knows|obviously|clearly|of course)\b/i,
    type: "bandwagon",
    description: "Appeal to common belief",
    mitigation: "Verify independently",
  },
  {
    pattern: /\b(the expert said|studies show)\b/i,
    type: "authority",
    description: "Over-reliance on authority",
    mitigation: "Evaluate the argument on merits",
  },
  {
    pattern: /\b(i've always|we've always|traditionally)\b/i,
    type: "anchoring",
    description: "Anchoring to historical practice",
    mitigation: "Consider fresh perspectives",
  },
  {
    pattern: /\b(already invested|sunk cost|too late to change)\b/i,
    type: "sunk_cost",
    description: "Sunk cost fallacy",
    mitigation: "Evaluate future value independently",
  },
  {
    pattern: /\b(just recently|in the news|viral|trending)\b/i,
    type: "availability",
    description: "Availability heuristic",
    mitigation: "Look for base rates and systematic data",
  },
  {
    pattern: /\b(confirms|proves my point|told you|knew it)\b/i,
    type: "confirmation",
...
function buildContradictionResolutionPrompt(
  userMessage: string,
  memoryContent: string,
): string {
  return [
    "Reconcile this contradiction using a concise, actionable response.",
    `Current message: "${userMessage.substring(0, 140)}"`,
    `Conflicting memory: "${memoryContent.substring(0, 140)}"`,
    "Task: Explain the likely relationship (change over time, context shift, or misunderstanding) and suggest how the assistant should proceed without being defensive.",
  ].join(" ");
}
...
const ENTITY_PATTERNS: Array<{ pattern: RegExp; type: SalientEntity["type"] }> =
  [
    ...
    {
      pattern: /\b(?:Google|Apple|Microsoft|Amazon|Meta|OpenAI|Tesla)\b/gi,
      type: "organization",
    },
    {
      pattern:
        /\b(?:AI|ML|API|SDK|CSS|HTML|SQL|REST|React|Python|JavaScript|TypeScript)\b/g,
      type: "technology",
    },
    ...
    for (const pair of negationPairs) {
      ...
      if ((userPos && memNeg) || (userNeg && memPos)) {
        contradictions.push({
          ...
          severity: 0.6,
          resolution: buildContradictionResolutionPrompt(
            userMessage,
            memory.content,
          ),
        });
...
export function buildInferenceChain(
  ...
  for (const memoryResult of memories.slice(0, 5)) {
    chain.push({
      premise: `User asks: "${userMessage.substring(0, 60)}"`,
      conclusion: `Relevant: "${memoryResult.memory.content.substring(0, 80)}"`,
      confidence: memoryResult.score,
      type: "abductive",
      supportingEvidence: [
        `Match: ${memoryResult.matchType}, Score: ${memoryResult.score.toFixed(3)}`,
      ],
    });
  }
  return chain;
...
export function buildReasoningFrame(
  ...
  const analogyMatch = userMessage.match(
    /\b(?:like|similar to|just as|analogous to)\b.*?(?:\.|$)/gi,
  );
  if (analogyMatch) {
  ...
  if (/\b(assuming|assume|presumably|suppose)\b/i.test(userMessage)) {
    assumptions.push({
      statement: userMessage.substring(0, 80),
      implicit: false,
      confidence: 0.7,
      risk: "Verify assumption",
    });
  }
  if (/\b(best|worst|always|never|everyone|no one)\b/i.test(userMessage)) {
    assumptions.push({
      statement: "Contains absolutes",
      implicit: true,
      confidence: 0.4,
      risk: "May not hold universally",
    });
  }
...
export function extractSalience(
  userMessage: string,
  memories: MemoryEntry[],
): SalienceMap {
  ...
  const keyActions = [
    ...new Set(
      (
        userMessage.match(
          /\b(create|build|fix|analyze|compare|explain|find|search|calculate|generate|write|design|implement)\b/gi,
        ) ?? []
      ).map((v) => v.toLowerCase()),
    ),
  ];
  ...
  const keyConstraints = (
    userMessage.match(
      /\b(must|should|need to|within|before|after|only|except|without)\b[^.!?]*/gi,
    ) ?? []
  )
    .slice(0, 3)
    .map((c) => c.trim());
  ...
  const emotionalHotspots = (
    userMessage.match(
      /\b(i feel|frustrated|anxious|happy|confused|stressed|excited)\b[^.!?]*/gi,
    ) ?? []
  )
    .slice(0, 2)
    .map((p) => p.trim());
  ...
  const informationDensity = Math.min(
    1,
    (entities.length / Math.max(words, 1)) * 5,
  );
  const focusPoint =
    entities.length > 0
      ? entities[0].text
      : (keyActions[0] ?? userMessage.substring(0, 40).trim());
  const peripheralContext = entities.slice(1, 4).map((e) => e.text);

  return {
    keyEntities: entities.slice(0, 6),
    keyActions,
    keyConstraints,
    emotionalHotspots,
    informationDensity,
...
export function buildReasoningInjection(frame: ReasoningFrame): string {
  ...
  if (frame.biases.length > 0) {
    const warnings = frame.biases
      .slice(0, 2)
      .map((b) => `- ${b.type}: ${b.description}. ${b.mitigation}`);
  ...
  if (frame.contradictions.length > 0) {
    parts.push(
      `### Contradictions\n${frame.contradictions
        ...
        .map(
          (c) =>
            `- "${c.claim1.substring(0, 50)}" vs "${c.claim2.substring(0, 50)}". ${c.resolution}`,
        )
        .join("\n")}`,
    );
  }
...
export function buildSalienceInjection(salience: SalienceMap): string {
  ...
  if (novel.length > 0)
    parts.push(
      `New entities: ${novel.map((e) => `"${e.text}" (${e.type})`).join(", ")}`,
    );
  if (salience.keyConstraints.length > 0)
    parts.push(`Constraints: ${salience.keyConstraints.join("; ")}`);
  if (salience.emotionalHotspots.length > 0)
    parts.push(`Emotional markers: ${salience.emotionalHotspots.join("; ")}`);
  if (salience.informationDensity > 0.7)
    parts.push(`High density. Focus: "${salience.focusPoint}"`);
