...
3. If you still get `EEXIST` / `EACCES` rename errors, run: `npm run build:prod:ios:local:repair`

These scripts set `NPM_CONFIG_CACHE=.npm-cache`, and the `:repair` variant also removes the global npm content cache (`~/.npm/_cacache`) before verifying npm cache state. This avoids permission collisions during local EAS builds.
