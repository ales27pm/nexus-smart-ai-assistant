      ...
      {Array.from({ length: 5 }).map((_, i) => (
        <div
          ...
          style={{
            ...
            animation: active ? `waveform 0.8s ease-in-out ${i * 0.15}s infinite` : 'none',
          }}
        />
      ))}
    </div>
...
export default function AudioDemo({ onBack }: Props) {
    ...
    if (!workerRef.current) {
      ...
      workerRef.current.addEventListener('message', (ev: MessageEvent) => {
        ...
        if (msgType === 'progress') {
          ...
          setProgress({ progress: pct, status: payload.file ?? payload.status ?? '' })
          ...
          const url = createWavBlob(payload.audio as Float32Array, payload.samplingRate as number)
          setTtsAudioUrl(url)
          setWorkerStatus('ready')
        } else if (msgType === 'interleaved_transcribed') {
          setInterleavedTurns((prev) => [...prev, { role: 'user', text: payload.inputText as string }])
          ...
          setInterleavedTurns((prev) => [
            ...prev,
            { role: 'assistant', text: payload.responseText as string, audioUrl: url },
          ])
          setStreamingAssistant('')
          setWorkerStatus('ready')
        }
  ...
  const getAudioFloat32 = async (blob: Blob): Promise<{ audio: Float32Array; sampleRate: number }> => {
    const arrayBuffer = await blob.arrayBuffer()
    const audioCtx = new AudioContext()
    const audioBuffer = await audioCtx.decodeAudioData(arrayBuffer)
    return { audio: audioBuffer.getChannelData(0), sampleRate: audioBuffer.sampleRate }
  }
  ...
  const handleASR = async () => {
    ...
    try {
      ...
      getWorker().postMessage({ type: 'transcribe', payload: { audio, sampleRate } }, [audio.buffer])
    } catch (e) {
      setError(String(e))
      setWorkerStatus('ready')
    }
  ...
  const handleTTS = () => {
    ...
    setWorkerStatus('processing')
    setTtsAudioUrl(null)
    getWorker().postMessage({ type: 'synthesize', payload: { text: ttsInput } })
  }

  const startRecording = async (chunksTarget: Blob[]) => {
    const stream = await navigator.mediaDevices.getUserMedia({ audio: true })
    const mr = new MediaRecorder(stream)
    mr.ondataavailable = (e) => { if (e.data.size > 0) chunksTarget.push(e.data) }
  ...
  const handleInterleavedRecord = async () => {
    if (isRecording) {
      ...
      mediaRecorderRef.current.onstop = async () => {
        ...
        try {
          const { audio, sampleRate } = await getAudioFloat32(blob)
          const history = interleavedTurns.map((t) => ({ role: t.role, content: t.text }))
      ...
      try {
        ...
        mediaRecorderRef.current = await startRecording(interleavedChunksRef.current)
        setIsRecording(true)
      } catch (e) {
        setError('Microphone access denied: ' + String(e))
      }
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        {workerStatus === 'loading' ? (
          <LoadingScreen progress={progress.progress} status={progress.status} modelName="LiquidAI/LFM2.5-Audio-1.5B-ONNX" />
        ) : (
          <div className="flex-1 flex flex-col items-center justify-center gap-8 p-8">
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold text-white mb-2">LFM2.5 Audio</h2>
              <p className="text-slate-400 max-w-md">
                Multimodal audio model for speech recognition, text-to-speech, and voice conversation — all running in your browser.
              </p>
            </div>
            <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-4 max-w-md">
              <ul className="text-sm text-slate-400 space-y-1.5">
                <li className="flex items-center gap-2"><span className="text-emerald-400">✦</span> ASR: speech to text</li>
                <li className="flex items-center gap-2"><span className="text-emerald-400">✦</span> TTS: text to speech</li>
                <li className="flex items-center gap-2"><span className="text-emerald-400">✦</span> Interleaved: voice conversation with LLM</li>
                <li className="flex items-center gap-2"><span className="text-emerald-400">✦</span> Model: LiquidAI/LFM2.5-Audio-1.5B-ONNX</li>
              </ul>
            </div>
            {error && (
    ...
    <div className="min-h-screen flex flex-col">
      <header className="border-b border-white/10 px-6 py-3 flex items-center gap-4 shrink-0">
        ...
        {workerStatus === 'processing' && (
          <span className="text-xs text-slate-500 ml-2 animate-pulse">Processing…</span>
        )}
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {mode === 'asr' && (
          <div className="max-w-2xl mx-auto space-y-6">
            <div className="text-center">
              <h2 className="text-lg font-semibold text-white mb-1">Speech Recognition</h2>
              <p className="text-sm text-slate-400">Upload an audio file or record from your microphone to transcribe speech to text.</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/3 p-6 flex flex-col items-center gap-4">
              <button
                onClick={isRecording ? handleStopRecordASR : handleStartRecordASR}
            ...
            <div>
              <input
                ...
                onChange={(e) => {
                  ...
                  if (f) { setAsrAudioFile(f); setRecordedBlob(null) }
                }}
              />
              <button
              ...
              >
                ...
                <span className="text-sm">{asrAudioFile ? asrAudioFile.name : 'Click to upload audio'}</span>
              </button>
            </div>
            <button
              onClick={handleASR}
              disabled={(!recordedBlob && !asrAudioFile) || workerStatus === 'processing'}
            ...
            >
              {workerStatus === 'processing' ? (
                <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Transcribing…</>
              ) : 'Transcribe'}
            </button>
            {asrText && (
              <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4">
                <p className="text-xs text-emerald-400 font-medium mb-2">Transcript</p>
                <p className="text-slate-200 text-sm whitespace-pre-wrap">{asrText}</p>
              </div>
            )}
          </div>
        )}
        ...
        {mode === 'tts' && (
          <div className="max-w-2xl mx-auto space-y-6">
            <div className="text-center">
              <h2 className="text-lg font-semibold text-white mb-1">Text to Speech</h2>
              <p className="text-sm text-slate-400">Type text and synthesize it to audio using the LFM2.5-Audio model.</p>
            </div>
            <textarea
              value={ttsInput}
            ...
            {ttsAudioUrl && (
              <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4">
                <p className="text-xs text-emerald-400 font-medium mb-2">Generated Audio</p>
        ...
        {mode === 'interleaved' && (
          <div className="max-w-2xl mx-auto space-y-4">
            <div className="text-center mb-4">
              <h2 className="text-lg font-semibold text-white mb-1">Voice Conversation</h2>
              <p className="text-sm text-slate-400">
                Speak to the model. It transcribes your voice, generates a reply, and speaks it back.
              </p>
            </div>
            ...
            {interleavedTurns.length === 0 && !streamingAssistant && (
              <div className="text-center py-12 text-slate-500">
                ...
                <p className="text-sm">Press the mic button to start speaking</p>
              </div>
            )}
              ...
              {interleavedTurns.map((turn, i) => (
                <div key={i} className={`flex gap-3 ${turn.role === 'user' ? 'justify-end' : ''}`}>
                  ...
                  <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm ${
                    ...
                    <p className={`text-xs mb-1 ${turn.role === 'user' ? 'text-emerald-200' : 'text-slate-500'}`}>
                ...
                <div className="flex gap-3">
                  <div className="w-8 h-8 rounded-full bg-emerald-500/20 border border-emerald-500/30 flex items-center justify-center shrink-0 mt-1">
                    <div className="flex gap-0.5">
                      {[0, 1, 2].map((i) => (
                        <div key={i} className="w-1 h-1 rounded-full bg-emerald-400 thinking-dot" style={{ animationDelay: `${i * 0.2}s` }} />
                      ))}
                    </div>
                  </div>
            ...
            <div className="flex flex-col items-center gap-3 pt-4">
              ...
              {!isRecording && interleavedTurns.length > 0 && workerStatus === 'ready' && (
                <button
