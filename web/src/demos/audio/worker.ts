...
const ASR_MODEL_ID = "onnx-community/whisper-tiny.en";
const LLM_MODEL_ID = "LiquidAI/LFM2.5-1.2B-Instruct-ONNX";
const TTS_MODEL_ID = "Xenova/speecht5_tts";
const TTS_VOCODER_MODEL_ID = "Xenova/speecht5_hifigan";
...
function postTransfer(msg: unknown, buffer: ArrayBufferLike): void {
  ...
  ).postMessage(msg, [buffer as ArrayBuffer]);
}
...
self.addEventListener("message", async (event: MessageEvent) => {
  ...
  if (type === "load") {
    ...
    try {
      ...
      asrPipeline = (await loadPipeline(
        "automatic-speech-recognition",
        ASR_MODEL_ID,
        baseOpts,
      )) as AutomaticSpeechRecognitionPipeline;
      llmPipeline = (await loadPipeline(
        "text-generation",
        LLM_MODEL_ID,
        baseOpts,
      )) as TextGenerationPipeline;
      ttsPipeline = (await loadPipeline("text-to-audio", TTS_MODEL_ID, {
        ...baseOpts,
        vocoder: TTS_VOCODER_MODEL_ID,
  ...
  if (type === "transcribe") {
    if (!asrPipeline) {
      self.postMessage({
        type: "error",
        payload: { message: "ASR model not loaded" },
      });
      return;
    }
    try {
  ...
  if (type === "synthesize") {
    if (!ttsPipeline) {
      self.postMessage({
        type: "error",
        payload: { message: "TTS model not loaded" },
      });
      return;
    }
    try {
      const { text } = payload as { text: string };
      const result = (await ttsPipeline(text, {})) as {
        audio: Float32Array;
      ...
      postTransfer(
        {
          type: "synthesized",
          payload: { audio: result.audio, samplingRate: result.sampling_rate },
        },
        result.audio.buffer,
      );
    } catch (err) {
  ...
  if (type === "interleaved") {
    if (!asrPipeline || !llmPipeline || !ttsPipeline) {
      self.postMessage({
        type: "error",
        payload: { message: "Models not loaded" },
      });
      return;
    }
    try {
      ...
      const messages = [
        {
          role: "system",
          content:
            "You are a helpful voice assistant. Give brief, natural spoken responses.",
        },
        ...history,
      ...
      const streamer = new TextStreamer(llmPipeline.tokenizer, {
        ...
        callback_function: (chunk: string) => {
          responseText += chunk;
          self.postMessage({
            type: "interleaved_token",
            payload: { token: chunk, fullText: responseText },
          });
        },
      });
      ...
      const ttsResult = (await ttsPipeline(responseText, {})) as {
        audio: Float32Array;
        sampling_rate: number;
      };
      postTransfer(
        {
          ...
          payload: {
            ...
            samplingRate: ttsResult.sampling_rate,
          },
        },
