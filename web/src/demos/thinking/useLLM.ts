...
export function useLLM(): UseLLMReturn {
  ...
  const loadModel = useCallback(async () => {
    ...
    return new Promise<void>((resolve, reject) => {
      const handler = (ev: MessageEvent) => {
        if (ev.data.type === 'loaded') { worker.removeEventListener('message', handler); resolve() }
        else if (ev.data.type === 'error') { worker.removeEventListener('message', handler); reject(new Error(ev.data.payload.message)) }
      }
  ...
  const generate = useCallback(async (messages: Array<{ role: string; content: string }>): Promise<string> => {
    const worker = getWorker()
  ...
  const abort = useCallback(() => {
    ...
    // Model is unloaded after termination — reset to idle so user can reload
    setStatus('idle')
    setStreaming('')
    resolveRef.current?.('')
  }, [])
